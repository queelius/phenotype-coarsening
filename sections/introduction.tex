...
literature that we do not claim to originate. In econometrics, the
non-identifiability of a distribution measured with unknown error is
the subject of partial identification: \citet{horowitz1995identification}
bound population parameters under corrupted sampling and
\citet{molinari2008partial} gives sharp regions for misclassified
distributions, of which our glass ceiling
(\cref{thm:glass-ceiling}) is the single-code instance.
...
rates supplied by the adjudicated subsample. Calibrated EHR estimators
that supply those error rates from a chart-reviewed sample already
exist (\citealp{tong2020augmented}; the SAMBA framework of
\citealp{beesley2022samba}; PhIAP, \citealp{zhang2019phiap}); our
deconvolution is their prevalence-only special case, derived as the
singleton-restoration step of the coarsening framework. \citet{hui1980estimating}
